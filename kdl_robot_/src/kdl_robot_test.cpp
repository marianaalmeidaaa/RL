    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            if (t <= init_time_slot) // wait a second
            {
                ...
                //p = planner.compute_trajectory_cubic_circular(0.0);
                //p = planner.compute_trajectory_cubic_linear(0.0);
            ...
            else if(t > init_time_slot && t <= traj_duration + init_time_slot)
            {
                ...
                //p = planner.compute_trajectory_cubic_circular(t-init_time_slot);
                //p = planner.compute_trajectory_cubic_linear(t-init_time_slot);
